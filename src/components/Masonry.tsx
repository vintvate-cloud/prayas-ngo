...
const Masonry = ({
  ...
  useLayoutEffect(() => {
    ...
    grid.forEach((item) => {
      const selector = containerRef.current?.querySelector(`[data-key="${item.id}"]`);
      if (!selector) return;
      ...
      if (!hasMounted.current) {
        ...
        gsap.fromTo(selector, initialState, {
          ...
          duration: 0.8,
          ease: 'power3.out',
          delay: (grid.indexOf(item)) * stagger
        });
      } else {
        gsap.to(selector, {
          ...animationProps,
  ...
  return (
    <div ref={containerRef} className="masonry-list" style={{ minHeight: `${maxContainerHeight}px` }}>
      {grid.map(item => {
        return (
          <div
            key={item.id}
          ...
          >
            <div className="masonry-item-img relative overflow-hidden" style={{ backgroundImage: `url(${item.img})` }}>
              <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4 text-white">
                {item.category && (
                  <span className="text-[10px] font-bold uppercase tracking-wider text-[#FFF314] mb-1">
                    {item.category}
                  </span>
                )}
                {item.title && (
                  <h4 className="text-sm font-bold leading-snug drop-shadow-md">
                    {item.title}
                  </h4>
                )}
              </div>
